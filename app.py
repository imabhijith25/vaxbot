...
def respond():
    try:
        ...
        if(update is not None and(update.message is not None ) ):
            ...
            if(db.getstate(session["info"]["chat_id"]) == "start"):
            ...
            elif(session["info"]["text"] == "Search By State" and db.getstate(session["info"]["chat_id"]) == "check" and (session["info"]["text"] != "/start" )):
            ...
            elif((session["info"]["text"].isalpha() == True) and (db.getstate(session["info"]["chat_id"])  == "state" and (session["info"]["text"] != "/start" ))):
            ...
            elif((session["info"]["text"].isalpha() == True) and (db.getstate(session["info"]["chat_id"])  == "district" and (session["info"]["text"] != "/start" ))):
                ...
                if(stval!=None):
                    ...
                    if(val == False):
                        ...
                        datt = "Enter the date on which you would like to get vaccinated.\nFormat DD-MM-YYYY\nEg: 02-02-2021"
            ...
            elif((session["info"]["text"].isalpha() == False) and (db.getstate(session["info"]["chat_id"])  == "date") and (session["info"]["text"] != "/start" )):
                ...
                if(distid_d is not None):
                    ...
                    if(j1 is not None):
                        ...
                        bot.sendMessage(chat_id=session["info"]["chat_id"],text="No Data Available")

                    continuetext1 = "would you like to continue?"
            ...
            elif((session["info"]["text"].isalpha() == False) and (db.getstate(session["info"]["chat_id"])  == "pin") and (session["info"]["text"] != "/start" )):
                #handlepincode  
                print("wat")
            ...
            elif((session["info"]["text"] == "Yes") and (db.getstate(session["info"]["chat_id"])  == "continue" and (session["info"]["text"] != "/start" ))):
        ...
        bot.sendMessage(chat_id=update.message.chat.id,text="Wrong format, please /start again")
...
def setwebhook():
    url = "https://052241de515d.ngrok.io/"
